...
def save_selected_sheet(n_clicks, selected_type, selected_sheet, contents, filename):
    if n_clicks >0 and selected_type is not None and selected_sheet is not None and contents is not None:
        ...
        try:
            ...
            # list of cells
            reference_cell_list = [
                df.iat[12,4],df.iat[12,5], #0) Package (or IFX Product)
                df.iat[14,4],df.iat[14,5], #1) Chip
                df.iat[15,4],df.iat[15,5], #2.1) Carrier(leadframe)
                df.iat[16,4],df.iat[16,5], #2.2) Carrier(Substrate)
                df.iat[17,4], #3) Die Attach(solder,epoxy,tape...)
                df.iat[18,4],df.iat[18,5], #4.1) Interconnect(wire)
                df.iat[19,4], #4.1.1) Wire bond method
                df.iat[20,4], #4.2) Interconnect(Clip)
                df.iat[21,4],df.iat[21,5], #4.3) Interconnect(Solder Bump)
                df.iat[22,4],df.iat[22,5], #4.4) Interconnect(Solder Ball)
                df.iat[23,4], #5) Encapsulant (resin,glob top, gel,...)
                df.iat[24,4], #6) Adhesion promoter
                df.iat[25,4], #7) Pre-Assembly Process
                df.iat[26,4], #8) FEOL process
                df.iat[27,4], #9) BEOL process
            ]

            target_cell_list = [
                df.iat[12,13],df.iat[12,14], #0) Package (or IFX Product)
                df.iat[14,13],df.iat[14,14], #1) Chip
                df.iat[15,13],df.iat[15,14], #2.1) Carrier(leadframe)
                df.iat[16,13],df.iat[16,14], #2.2) Carrier(Substrate)
                df.iat[17,13], #3) Die Attach(solder,epoxy,tape...)
                df.iat[18,13],df.iat[18,14], #4.1) Interconnect(wire)
                df.iat[19,13], #4.1.1) Wire bond method
                df.iat[20,13], #4.2) Interconnect(Clip)
                df.iat[21,13],df.iat[21,14], #4.3) Interconnect(Solder Bump)
                df.iat[22,13],df.iat[22,14], #4.4) Interconnect(Solder Ball)
                df.iat[23,13], #5) Encapsulant (resin,glob top, gel,...)
                df.iat[24,13], #6) Adhesion promoter
                df.iat[25,13], #7) Pre-Assembly Process
                df.iat[26,13], #8) FEOL process
                df.iat[27,13], #9) BEOL process
            ]
            ...
            reference_values = []
            target_values = []
            #To check any blank cell and replace with NA
            reference_cell_list = ['NA' if pd.isna(content) else content for content in reference_cell_list]
            target_cell_list = ['NA' if pd.isna(content) else content for content in target_cell_list]
            # Extract reference data in the list
            for i,reference_cell_content in enumerate(reference_cell_list):
                # Split the cell content into lines

                if i != 11:  # Index 11 corresponds to df.iat[18,4] #Wire bon method
                    reference_lines = reference_cell_content.strip().split('\n')
                else:
                    reference_lines = [reference_cell_content.strip()]
            ...
            for i,target_cell_content in enumerate(target_cell_list):
                # Split the cell content into lines
                if i != 11:
                    target_lines = target_cell_content.strip().split('\n')
                else:
                    target_lines = [target_cell_content.strip()]
            ...
            print(e)
            return html.Div('Error, Plese check delta analysis format')
    else:
...
#xls = r"\\SINSDN38.ap.infineon.com\SIN_QM1\14_REL\09-PPQ\Lean Qual Referencing\WP1\Z8F52119328 (Delta Analysis Template)_updated.xlsm"
#df = pd.read_excel(xls, sheet_name='Delta Assessment (Trial 1)')
#output_path=r'C:/temp/output_Delta Assessment.xlsx'
